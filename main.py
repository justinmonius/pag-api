...
# ================================================================
# STEP 1: MAIN PROCESS ENDPOINT (CREATES UPDATED PAG)
# ================================================================
@app.post("/process")
async def process_files(
    ...
    for name in [
        ...
        if all(col in df.columns for col in ["(a)P/N&S/N", "PO Number", "Ship Date", "(f) Qty"]):
            df = df[["(a)P/N&S/N", "PO Number", "Ship Date", "(f) Qty", "(g) Unit/Lot (Repair) Price)"]].copy() \
                if "(g) Unit/Lot (Repair) Price)" in df.columns else df[["(a)P/N&S/N", "PO Number", "Ship Date", "(f) Qty"]]
    ...
    if ebu_frames:
        ...
        for (part, po), cutoff_date in ship_latest_dates.items():
            if pd.notna(cutoff_date):
                mask = (ebu_df["Part #"] == part) & (ebu_df["PO Number"] == po) & (ebu_df["Ship Date"] > cutoff_date)
    ...
    # ---------- SUMMARY SHEETS ----------
    latest_po_part = pag_output.groupby(["Material", "Purchasing Document"])["Stat.-Rel. Del. Date"].max().reset_index()
    ship_counts = pd.DataFrame(list(shipped_map.items()), columns=["(Material, PO)", "Shipment_Count"])
    ebu_counts_df = pd.DataFrame(list(ebu_counts.items()), columns=["(Material, PO)", "EBU_Count"])

    # ---------- WRITE OUTPUT ----------
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        pag_output.to_excel(writer, index=False, sheet_name="Updated")
        latest_po_part.to_excel(writer, index=False, sheet_name="Latest_Dates")
        ship_counts.to_excel(writer, index=False, sheet_name="Shipment_Totals")
        ebu_counts_df.to_excel(writer, index=False, sheet_name="EBU_Totals")
...
async def delta_report(
    ...
    # Extract price map from all EBU sheets
    price_map = []
    for name, df in ebu_sheets.items():
        df.rename(columns=lambda x: str(x).strip(), inplace=True)
        if "(g) Unit/Lot (Repair) Price)" in df.columns and "(a)P/N&S/N" in df.columns and "PO Number" in df.columns:
            temp = df[["(a)P/N&S/N", "PO Number", "(g) Unit/Lot (Repair) Price)"]].copy()
            temp.rename(columns={"(a)P/N&S/N": "Material", "PO Number": "Purchasing Document", "(g) Unit/Lot (Repair) Price)": "Unit_Price"}, inplace=True)
            price_map.append(temp)
    price_df = pd.concat(price_map, ignore_index=True).drop_duplicates(subset=["Material", "Purchasing Document"])
    ...
    for df in [new_df, old_df]:
        ...
        if not possible_cols:
            raise ValueError("Could not find Stat.-Rel. Del. Date column.")
    ...
    # ---------- Grouping ----------
    new_grouped = new_df.groupby(["Material", "Purchasing Document", "Month"])["Qty remaining to deliver"].sum().reset_index().rename(columns={"Qty remaining to deliver": "New_Qty"})
    old_grouped = old_df.groupby(["Material", "Purchasing Document", "Month"])["Qty remaining to deliver"].sum().reset_index().rename(columns={"Qty remaining to deliver": "Old_Qty"})

    merged = pd.merge(new_grouped, old_grouped, on=["Material", "Purchasing Document", "Month"], how="outer").fillna(0)
    merged["Delta"] = merged["New_Qty"] - merged["Old_Qty"]

    # ---------- Pivot ----------
    pivot = merged.pivot_table(index=["Material", "Purchasing Document"], columns="Month", values="Delta", fill_value=0).reset_index()
    pivot.columns.name = None
    sorted_cols = ["Material", "Purchasing Document"] + sorted([c for c in pivot.columns if c not in ["Material", "Purchasing Document"]])
    ...
    for i, col in enumerate(month_cols):
        if i == 0: continue
        cumulative[col] = cumulative[month_cols[:i+1]].sum(axis=1)
    ...
    revenue_pivot = merged_price.pivot_table(index=["Material", "Purchasing Document"], columns="Month", values="Revenue", fill_value=0).reset_index()
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        revenue_pivot.to_excel(writer, index=False, sheet_name="Revenue_Delta")
